...
namespace Cust.DAL
{
	...
	public partial class customerDal
	{
		...
		public Cust.Model.customer GetModel(int Id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 Id,Name,Note from customer ");
			strSql.Append(" where Id=@Id");
			SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.Int,4)
			};
			...
			Cust.Model.customer model=new Cust.Model.customer();
		...
		/// 得到一个对象实体
		/// </summary>
		public Cust.Model.customer DataRowToModel(DataRow row)
		{
			Cust.Model.customer model=new Cust.Model.customer();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select Id,Name,Note ");
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("SELECT TT.Id,tt.Name,tt.Note FROM ( ");
